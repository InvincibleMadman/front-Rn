...
export function AssetLineageGraph({
  ...
  return (
    <AssetGraphViewportLayout
      toolbar={toolbar}
      aside={(
        <aside className="flex h-full min-h-0 min-w-0 flex-col rounded-[var(--radius-lg)] border border-border bg-card p-4 shadow-console">
          ...
          {selectedRelation ? (
            <div className="console-scrollbar mt-4 min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
              <div>
              ...
              <div className="space-y-2">
                <div className="rounded-[var(--radius-lg)] border border-border/60 bg-background/70 px-3 py-2">
            ...
            <div className="console-scrollbar mt-4 min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
              ...
              <div className="space-y-2">
                {selectedEntity.attributes.map((row, index) => (
                  <div key={`${selectedEntity.id}-${row.key}-${index}`} className="rounded-[var(--radius-lg)] border border-border/60 bg-background/70 px-3 py-2">
            ...
            <div className="mt-4 flex min-h-0 flex-1 items-center justify-center rounded-[var(--radius-lg)] border border-dashed border-border/70 bg-background/50 px-4 text-center text-sm text-muted-foreground">
              选择 UML 实体或关系以查看详情。
            </div>
          )}
        </aside>

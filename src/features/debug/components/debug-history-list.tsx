  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <div className="mb-4 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">历史会话</p>
          <p className="mt-1 text-sm text-foreground">重新打开之前的调试会话，并回填启动参数。</p>
        </div>
        <div className="flex flex-col gap-2 md:flex-row">
          <div className="relative min-w-[16rem]">
            <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
            <Input value={keyword} onChange={(e) => onKeywordChange(e.target.value)} className="pl-9" placeholder="搜索会话 / 函数 / 文件 / 类型" />
          </div>
          ...
          return (
            <div key={session.session_id} className="rounded-lg border border-border bg-background px-4 py-4">
              <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge variant="outline" className="rounded-md">{report?.coarse_type || report?.vuln_type || "未知"}</Badge>
                    {report?.cwe ? <Badge variant="outline" className="rounded-md">{report.cwe}</Badge> : null}
                  </div>
                  <p className="mt-3 text-base font-semibold text-foreground">
                    {location?.primary_function || report?.root_cause || "调试会话"}
                  </p>
                  <p className="mt-1 break-all text-sm text-muted-foreground">
                    {location?.primary_file_path || report?.vulnerability_location?.file_path || "暂无"}
              ...
              <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-muted-foreground">
                ...
                <span>样本: {session.request?.artifact_id || "暂无"}</span>
                <span>更新: {formatDateTime(session.updated_at || session.created_at)}</span>
              </div>
            </div>
          );
        }) : (

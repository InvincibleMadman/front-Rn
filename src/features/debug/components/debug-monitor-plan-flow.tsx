  ...
  return (
    <div className="rounded-md border border-border bg-card p-3">
      <div className="mb-2 flex items-center justify-between gap-3 border-b border-border pb-2">
        <p className="text-[12px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">智能体任务计划</p>
        <p className="text-[11px] text-muted-foreground">Plan / Observe / Reason</p>
      </div>
      ...
      <div className="overflow-hidden rounded border border-border bg-background">
        {items.length ? items.map((item, index) => (
          <div key={`${item.at ?? 'step'}-${index}`} className="grid grid-cols-[2rem_minmax(0,4.8rem)_minmax(0,1fr)_4.5rem] gap-3 border-b border-border px-3 py-2 last:border-b-0">
            ...
            <div className="text-right text-[11px] text-muted-foreground">{item.at ? formatDateTime(item.at) : "—"}</div>
          </div>
        )) : (
          <div className="px-3 py-8 text-[14px] text-muted-foreground">当前还没有任务计划输出。</div>
        )}
      </div>
    </div>
  );
